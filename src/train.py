...
def main(args, resume_preempt=False):
    ...
    root_path = args['data'].get('root_path')
    ...
    if dataset_type == 'video_first_frame':
        _, unsupervised_loader, unsupervised_sampler = make_video_frame_loader(
            transform=transform,
            ...
            drop_last=True)
    elif dataset_type == 'imagenet_plus_video_first_frame':
        _, unsupervised_loader, unsupervised_sampler = make_imagenet_and_video_frame_loader(
            transform=transform,
            batch_size=batch_size,
            collator=mask_collator,
            pin_mem=pin_mem,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, (udata, masks_enc, masks_pred) in enumerate(unsupervised_loader):
            ...
            def load_imgs():
                # -- unsupervised img pairs
                img1 = udata[0].to(device, non_blocking=True)
                img2 = udata[1].to(device, non_blocking=True)
                masks_1 = [u.to(device, non_blocking=True) for u in masks_enc]
                masks_2 = [u.to(device, non_blocking=True) for u in masks_pred]
                return (img1, img2, masks_1, masks_2)
            imgs1, imgs2, masks_enc, masks_pred = load_imgs()
            ...
            def train_step():
                ...
                def forward_context():
                    z_q = encoder(imgs2, masks_enc)
                    z_kv = encoder(imgs1, masks=None)
                    z = predictor(z_q, z_kv, masks_enc, masks_pred)
                    return z
